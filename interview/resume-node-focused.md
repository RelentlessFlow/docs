...
- 掌握 JavaScript (ES6+)、TypeScript，深入理解类型系统并能进行复杂类型操作
- 熟练使用 **NestJS** 框架构建企业级后端应用，熟悉模块化架构、依赖注入、装饰器模式
- 熟练使用 **TypeORM**、**Prisma** 等 ORM 框架进行数据库操作
- 熟悉 **PostgreSQL (pgvector)** 向量数据库、**MySQL** 关系型数据库设计、SQL 优化
- 熟悉 **Elasticsearch** 全文检索、聚合分析、多维度统计
- 熟悉 **Redis** 缓存应用场景及常用数据结构
- 熟悉 **MinIO** 对象存储服务
- 熟悉 **RabbitMQ**、**BullMQ** 消息队列的应用

### 前端

- 掌握 **Vue 3** 生态（Composition API、Vue Router、Pinia）
- 掌握 **React** 及 Hooks 开发模式
- 熟练使用 **UnoCSS**、**Tailwind CSS** 原子化 CSS 框架
- 熟悉 **Quasar**、**Ant Design**、**Arco Design** 等 UI 组件库

### 桌面端

- 熟练使用 **Electron** 进行跨平台桌面应用开发
- 熟悉主进程、渲染进程、预加载脚本架构
- 熟悉 IPC 通信、原生菜单、系统托盘、自动更新、崩溃日志采集等功能实现

### 工程化

- 熟练使用 **pnpm workspace** 管理 Monorepo 项目
- 熟悉 **Turborepo** 构建优化
- 熟悉 **Vite**、**Webpack** 构建工具配置与优化
- 熟练使用 Git 进行协作开发，熟悉 Git Flow 工作流
- 熟悉 Docker 容器化部署
- 熟悉 CI/CD 自动化部署流程
...
**Monorepo 架构设计**
- 负责 Monorepo 架构设计，管理 types、utils 共享包及多个应用项目
...
**PostgreSQL 向量检索**
- 实现 pgvector 向量检索，支持语义搜索、相似度匹配
- 设计 Ranking 重排序算法，优化检索结果质量
- 实现 LRU 缓存机制，缓存工具生成的 SQL，优化重复查询响应速度

**MinIO 文件服务**
- 开发 MinIO 文件服务，实现文件上传、下载、权限控制
- 实现多存储桶管理，支持按业务隔离文件存储
...
企研数据第一代产品，集成 AI 对话功能的社科大数据平台，为高校师生提供数据检索、云桌面、AI 辅助研究等服务。采用 Monorepo 架构。
...
**后端架构**
- 开发 NestJS 后端服务，实现 RESTful API、JWT 认证、基于角色的访问控制
- 封装通用 DTO/Entity 基类，减少重复代码，统一数据格式
...
面向社会科学研究者的 AI 数据分析桌面应用，用户通过自然语言描述分析需求，自动生成并执行 Python 代码。采用 Electron + React 技术栈。

技术栈

Electron + React + TypeScript + NestJS (管理后台) + Monaco Editor + Tiptap

核心难点与负责内容

**Electron 主进程架构**
- 设计 Electron 主进程与渲染进程 IPC 通信架构，实现类型安全的 IPC 调用
- 实现渲染进程崩溃处理、IPC 类型安全通信
...
**管理后台**
- 开发 NestJS 管理后台，实现用户管理、积分系统、公告管理
...
- 实现容器资源隔离、磁盘容量配置
- 解决未配置容器存储空间大小环境变量时，无法进入 Jupyter 的 Bug
...
基于开源 99AI 项目的 AI 对话平台，接入公司内部用户系统。
